...
export class FoodEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.foodId = Number(routedParams.get('foodId'));   
  ...
  getFoodDetails() {
    this.foodService
      ...
      .subscribe((data) =>{
        this.patchFoodItem(data);
        this.foodItem = data;
      });
  }


  private patchFoodItem(foodMenu:FoodMenu){
    
    this.control('name')?.patchValue(foodMenu.name);
    this.control('description')?.patchValue(foodMenu.description);
    this.control('isActive')?.patchValue(foodMenu.isActive);
    this.control('categoryId')?.patchValue(foodMenu.categoryId);
    this.control('cuisineId')?.patchValue(foodMenu.cuisineId);
    this.control('price')?.patchValue(foodMenu.price);
  }
  ...
  saveFoodItem() {
    ...
    if (this.form.valid) {
      ...
      this.foodService.updateFoodItem(model).subscribe(
        () => this.onSaveComplete(),
